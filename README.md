...
### 设计编辑器
- 数据驱动画布渲染，支持 frame / group / rectangle / text / chart / comment / path / image 八类图层
- 四区工作台：顶部工具栏、左侧图层面板（图层 / 组件两个 tab）、中央无限画布、右侧检视器（设计 / 原型 / 检查三个 tab）；左右侧栏可拖拽调整宽度并自适应
- 绘制工具：画板、矩形、路径（折线绘制）、文本、评论；快捷键键帽展示（V / F / R / P / T / C）
- 组件库内置 **24 种模板**（按钮 / 输入框 / 图表 / 导航栏 / 卡片 / 头像 / 进度条 / 开关等），拖拽/点击插入；支持**自定义组件**：把画布分组保存为组件本地复用
- **组件个性化（schema 驱动）**：检视面板按模板声明自动生成表单，直接编辑组件属性
  - 主题预设一键切换（按钮 5 主题、卡片 / 导航栏深浅色）
  - 渐变背景、内部布局参数、图片填充方式与圆角
  - 组件状态（hover / pressed / disabled / loading / error），预览态支持悬停 / 按下演示
- **图表组件**：柱 / 折线 / 面积 / 饼 / 环形五类型 SVG 渲染；数据逐项独立编辑（每柱 / 每扇区数值 + 独立配色、增删数据项）；数值标注 / 图例开关
- 多页文档管理：新建 / 重命名 / 复制 / 删除页面，画板尺寸预设
- 选择能力：框选（Shift + 拖空白）、Shift 多选、多选整体拖拽、Shift 等比缩放、拖拽吸附（6px）、Alt 间距标注
- 画布交互：滚轮缩放、空格 / 中键平移、适应画布、旋转手柄（15° 吸附）
- 属性编辑：X/Y/W/H、旋转、圆角、填充、透明度、描边、阴影、文本样式、图片上传（自动压缩降分辨率）、图表数据、评论内容
...
- 项目列表首页：新建、复制、归档 / 取消归档、物理删除、导入 `.xgproj` 项目文件
...
| `CORS_ALLOWED_ORIGINS` | `http://localhost:5183` | 允许的跨域来源（逗号分隔多个） |
...
│   │   ├── constants/        #   colors / labels / limits（全局常量）
│   │   ├── hooks/            #   useKeyboardShortcuts 等
│   │   ├── utils/            #   geometry / chart / export / inspect / image / customComponents
│   │   └── fixtures/         #   starter-document / component-library（组件模板）
