...
export function ShareModal({ projectId, onClose }: Props) {
  ...
  const startShare = () => {
    const share = { link: `${window.location.origin}${window.location.pathname}#/doc/${projectId}`, permission, active: true, createdAt: Date.now() }
    localRepository.setShare(projectId, share)
    setActive(true)
  }

  const updatePermission = (p: Permission) => {
    setPermission(p)
    if (active) localRepository.setShare(projectId, { link: `${window.location.origin}${window.location.pathname}#/doc/${projectId}`, permission: p, active: true, createdAt: Date.now() })
  }

  const revoke = () => {
    localRepository.setShare(projectId, null)
    setActive(false)
  }
        ...
        <div className="modal-body">
          {!active ? (
            <>
              ...
              <button className="export-button" onClick={startShare}>创建分享链接</button>
            ...
            <>
              ...
              <button className="share-revoke" onClick={revoke}>取消分享</button>
